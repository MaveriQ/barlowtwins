...
class BookCorpusDataModuleForMLM(pl.LightningDataModule):
    def __init__(self, args):
        ...
        # print(f'caching status : {datasets.is_caching_enabled()}')
        if self.args.dataset == '1mil':
            ...
            # print(f"dataset fingerprint : {onemil_dataset._fingerprint}")
            self.dataset = onemil_dataset['train'].map(lambda e: self.tokenizer(e['text'],return_token_type_ids=False,truncation=True,padding='max_length',max_length=self.args.seq_len),remove_columns=['text'],num_proc=16)
            print(f"dataset fingerprint : {self.dataset._fingerprint}")
        else:
            ...
            if self.args.dataset == '5mil':
                self.dataset = bookcorpus.select(range(5000000)).map(lambda e: self.tokenizer(e['text'],return_token_type_ids=False,truncation=True,padding='max_length',max_length=self.args.seq_len),remove_columns=['text'],num_proc=16)
            elif self.args.dataset == '10mil':
                self.dataset = bookcorpus.select(range(10000000)).map(lambda e: self.tokenizer(e['text'],return_token_type_ids=False,truncation=True,padding='max_length',max_length=self.args.seq_len),remove_columns=['text'],num_proc=16)
